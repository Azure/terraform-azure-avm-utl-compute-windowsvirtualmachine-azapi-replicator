...
- ❌ **Failed**: Analyze error before marking invalid

**If apply fails, check for random resource constraint violations**:

Common error patterns:
- `expected length to be in the range (X - Y), got Z`
- `storage account name must be between 3 and 24 characters`
- `computer name cannot be more than 15 characters`
- `name must match pattern [regex]` or `must contain only alphanumeric characters`

**If error matches above patterns**:
1. Edit `main.tf` to adjust the `random_string` length or attributes (e.g., set `upper = false`, `special = false`)
2. Common fixes: computer name ≤15 chars, storage account 3-24 lowercase alphanumeric
3. Retry apply (max 5 attempts)
4. Document fix in `random_fix.log`: `Test Case: [name] | Error: [message] | Fix: [changes] | Result: Success/Failed`

**If fix succeeds**: Continue to verify idempotency  
**If fix fails after 5 attempts OR error is not random-resource related**: Run destroy cleanup, mark test case as `invalid` in `test_cases.md`, QUIT
