...
### 5. Post-Creation Operations - Sequential Pattern

After primary resource creation, the provider may invoke additional SDK operations. Query Create method to identify operations AFTER primary `CreateOrUpdateThenPoll`, then classify by SDK function type:

**Classification & Indexing:**
- **CreateOrUpdate/Create** → `post_creation<idx>` (child resource, different type/name/parent_id)
- **Update** → `post_update<idx>` (update to main resource, same type)
- Index sequentially across all types: `post_creation0`, `post_update1`, `post_update2`

**Structure (object, not list):**
```hcl
locals {
  post_creation0 = var.field != null ? {
    azapi_header = { type = "Microsoft.Network/.../SOA@2024-06-01" }  # Child resource type
    body = { properties = {...} }; locks = local.locks
  } : null
  
  post_creation0_sensitive_body = var.field != null ? {
    properties = {...}  # Sensitive/WriteOnly fields only
  } : null
  
  post_update1 = var.other_field != null ? {
    azapi_header = { type = "..." }  # Same as local.azapi_header.type
    body = { properties = {...} }; locks = local.locks
  } : null
  
  post_update1_sensitive_body = var.other_field != null ? {
    properties = {...}  # Sensitive/WriteOnly fields only
  } : null
}
```
...
**Steps:** 
1. Query Create with `query_terraform_block_implementation_source_code`
2. **Check Multi-Phase**: Identify ALL SDK operations after primary create, classify each (CreateOrUpdate/Update), assign indices
3. Find hardcoded values NOT in schema
4. **Check locks** (see below)
5. Add hidden fields to `local.body.properties`, add to `local.locks`
6. **If post-operations**: Document pattern for Type 5 tasks
7. Create proof, update `track.md` to `Pending for check`

**Pattern Examples:** Single (`CreateOrUpdate` only) | Two-phase (`Create` → `RecordSetsCreateOrUpdate` = `post_creation0`) | Mixed (`Create` → `ChildCreateOrUpdate` → `Update` = `post_creation0`, `post_update1`)
...
### Type 5: Post-Creation Operation
**When:** Field processed AFTER primary Create operation.
**Steps:** 
1. Confirm operation after primary `CreateOrUpdateThenPoll`
2. **Identify SDK function** (CreateOrUpdate → `post_creation<idx>` | Update → `post_update<idx>`)
3. **Count index**: Sequential across all post-operations (0-based)
4. Query schema/API, **IMPLEMENT validations** (MANDATORY)
5. Add to appropriate local (object, not list)
6. Set `azapi_header.type`: child resource type for post_creation, main type for post_update
7. Handle sensitive, create proof, update `track.md` to `Pending for check`
...
2. Identify: Single-phase (one `CreateOrUpdate`) vs Multi-phase (primary + additional operations)
3. For multi-phase: Extract each SDK function name, classify (Create/Update), assign sequential index
4. Classify field: Before primary create → `local.body` | After in CreateOrUpdate → `local.post_creation<idx>` | After in Update → `local.post_update<idx>`
5. Document in proof with Go evidence and SDK function names
...
locals {
  ...
  # Post-creation operations (if any) - populated by Type 2/Type 5 tasks
  # post_creation0 = null  # Example: First CreateOrUpdate after primary create
  # post_creation0_sensitive_body = null  # Sensitive fields for post_creation0
  # post_update1 = null     # Example: First Update after primary create
  # post_update1_sensitive_body = null  # Sensitive fields for post_update1
  # post_update2 = null     # Example: Second Update after primary create
  # post_update2_sensitive_body = null  # Sensitive fields for post_update2
